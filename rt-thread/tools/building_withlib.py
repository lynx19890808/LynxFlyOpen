...
def PrepareBuilding(env, root_directory, has_libcpu=False):
    ...
    global Env
    global Rtt_Root
    global BuildGroupAsLib

    Env = env
    Rtt_Root = root_directory

    # patch for win32 spawn
    if env['PLATFORM'] == 'win32' and rtconfig.PLATFORM == 'xcc':
    ...
    AddOption('--lib',
                      ...
                      action='store',
                      default="",
                      help='compile group to library')
    if GetOption('library'):
        building_root = str(Dir("#"))
        BuildGroupAsLib = True
        env.Append(LIBPATH=os.path.join(building_root, "build", "lib"))
        env.Append(LIBS=['c', 'm'])
...
def DefineGroup(name, src, depend, **parameters):
    ...
    if BuildGroupAsLib:
        if group.has_key('LIBRARY') and group['LIBRARY'] == False:
            objs = objs
        else:
            building_root = str(Dir('#'))
            # objs = Env.Library(os.path.join("lib", name), objs)
            Env.Library(os.path.join(building_root, "build", "lib", name), objs)
