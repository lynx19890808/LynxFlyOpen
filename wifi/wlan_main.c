...
extern int Wlan_Association(WlanConfig* config, WlanInfo * wlaninfo);
extern int gspihost_init(void);
extern int set_wep_no_key(WlanCard *cardinfo);
...
{
	{"Dennis_wifi", "1234567890", -1},
	{"ChinaNet-Dze7", "zhdfzghp", -1},
	{"rtthread", "12345678", -1}
};
...
void wlan_send_rxskbQ(WlanCard * card)
{
	...
	{
		eth_device_ready(&(netdev->parent));
	}
}
...
int WlanInitPhase1(struct rt_wlan_dev* RTWlanDev)
{
	rt_err_t result;
	...
	cardinfo.wlandev=RTWlanDev;
	RTWlanDev->priv=(void*)&wlaninfo;
	...
	result= rt_thread_init(&WlanRxThread,
			...
			wlan_rx_thread,
			RT_NULL,
			(rt_uint8_t*)&main_thread_stack[0],
			sizeof(main_thread_stack), 10, 100);
	if (result == RT_EOK) rt_thread_startup(&WlanRxThread);

	result=wlan_init_card(&cardinfo);
	if(result!=0) return -1;

	result = gspihost_init();
	if(result<0)
	{	
...
{
	WlanCard *card=&cardinfo;
	char macadd[18]={0x33,0x33,0x00,0x00,0x00,0x01,0x01,0x00,0x5e,0x00,0x00,0x01,0x33,0x33,0xff,0xa7,0x12,0xc0};

	Multi_Addr_Struct mac_address_arry;
	
	SendSpecificBSSIDScan(card, config->MacAddr,(char *)config->SSID,config->channel);

	rt_memcpy((u8*)&mac_address_arry.multi_addr,macadd,18);
	mac_address_arry.addressnum=3;
	// wlan_set_mac_multicast_add(card, &mac_address_arry);
	wlan_set_infrastructure(card);

	return 0;
}

//猞猁重写了该函数以保证返回值能携带连接状态信息
// int WlanStartAssociation(WlanConfig* config)
// {
// 	WlanCard *card = &cardinfo;
...
// 		memcpy(&keyArray.Key_value[0][0], specialconfig->password, 5);

// 		keyArray.KeyLength[0] = 5;
...
int WlanStartAssociation(WlanConfig* config)
{
	...
	if (config->security == WEP)
	{
		...
		keyArray.KeyLength[0] = 5;
		ret = set_wep_materials(card, &keyArray);
		if(ret == WLAN_STATUS_SUCCESS)
			ret = wlan_set_wap(card, specialconfig->MacAddr);
		else
			return ret;		
	}
	else if (config->security == WPA_PSK || config->security == WPA2_PSK)
	{
		wlan_set_wpa_info(specialconfig, &wlaninfo);
		if(ret == WLAN_STATUS_SUCCESS)
			ret = Wlan_Association(config, &wlaninfo);
		else
			return ret;
	}
	else if (config->security == NoSecurity)
...
void WlanScanProbe(void)
{
	...
	ret = SendScanToGetAPInfo(card, (char *)ptr, 1024);
	if (ret != 0)
...
{
	/* set security */
	wlanconfig.security = (SecurityMode)security_mode;
	
	/* copy ssid */
	rt_strncpy((char *)wlanconfig.SSID, ssid, sizeof(wlanconfig.SSID));

	if (security_mode == NoSecurity)
		return;

	/* copy password */
	rt_strncpy((char *)wlanconfig.password, passwd, sizeof(wlanconfig.password));
}
...
int WlanDirectConnect()
{
	int channel = 11;
  char *ssid = "rtthread";
	char mac[6] = {0x4c, 0x80, 0x93, 0x4d, 0x1a, 0xdf};   //电脑热点参数

// 	int channel = 11;
//   char *ssid = "LynxGround";
// 	char mac[6] = {0xa8, 0x26, 0xd9, 0xc2, 0xfd, 0x13};  //手机热点参数

	wlan_set_ap_info(channel, (rt_uint8_t *)mac);
	wlan_set_security(NoSecurity, ssid, RT_NULL);
	
	WlanScanAP(&wlanconfig);
	return WlanStartAssociation(&wlanconfig);

	/* notify interface link up */
	// rt_wlan_linkchange(RT_TRUE);

	//return 0;
}

/*
*这是后来额外加入的直接通过SSID连接的方法 需要用到的几个全局变量 啊啊啊啊 实在懒得改成函数参数的形式了
*/
char *Default_SSID;
char *cnt_ssid;
char cnt_mac[6];
int cnt_channel=11; //随便给的默认参数 防止意外出错的
/*
*这是后来额外加入的连接方法，需要事先通过程序找到无密码的热点的三个信息
*/
int WlanConnectTo()
{
	wlan_set_ap_info(cnt_channel, (rt_uint8_t *)cnt_mac);
	wlan_set_security(NoSecurity, cnt_ssid, RT_NULL);
	
	WlanScanAP(&wlanconfig);
	if(WlanStartAssociation(&wlanconfig))
		return -1;
...
int ConnectSSID(char *connssid)
{
	...
	ScanResultItem * item;
	int i=0;
	if((!strcmp(connssid,""))||(connssid==RT_NULL))
	{
		cnt_ssid=Default_SSID;
	}
	else
	{
		cnt_ssid=connssid;
	...
	{
		item = card->ScanResultInfo + i;
		if(!strcmp(cnt_ssid,(char *)item->Ssid.Ssid))
		{
			rt_memcpy(cnt_mac,item->MacAddress,6);
			cnt_channel = (int)(item->Channel);
//			rt_kprintf("Connecting: \"%s\"... (0x%X,0x%X,0x%X,0x%X,0x%X,0x%X)\r\n",ssid,mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
			WlanConnectTo();  
			return 0;
		}
	}
	rt_kprintf("SSID: \"%s\" not found,please try again\r\n",cnt_ssid);
	return -1;
}
...
void wlan_status(void)
{
	...
	rt_kprintf("ChipID: 0x%02x\n", card->ChipID);
	rt_kprintf("RxQueueCount: %d\n", card->RxQueueCount);
	rt_kprintf("HisEvent: 0x%02x\n", card->HisRegCpy);
}
FINSH_FUNCTION_EXPORT(wlan_status, get wlan status);
#endif
