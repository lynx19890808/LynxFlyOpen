...
static int wlan_scan_channel_list(WlanCard *cardinfo,int maxChanPerScan,BOOLEAN filteredScan,wlan_scan_cmd_config * pScanCfgOut,MrvlIEtypes_ChanListParamSet_t * pChanTlvOut,ChanScanParamSet_t * pScanChanList)
{
	WlanCard *card=cardinfo;
 ...
 *  @return        WLAN_STATUS_SUCCESS or WLAN_STATUS_FAILURE
 */
int wlan_ret_802_11_scan(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp)
{
    WlanCard *card=cardinfo;
    ...
    //newBssEntry=rt_malloc(sizeof(BSSDescriptor_t));
    newBssEntry=(BSSDescriptor_t *)scanbuffer;
   
    if(newBssEntry==NULL)
    {
	 WlanDebug(WlanErr,  "wlan_ret_802_11_scan alloc memory failed\n");
	 return -1;
    }
    if (pScan->NumberOfSets > MRVDRV_MAX_BSSID_LIST) {
        WlanDebug(WlanErr, "SCAN_RESP: Invalid number of AP returned (%d)!!\n",
               pScan->NumberOfSets);
        return WLAN_STATUS_FAILURE;
    }
    ...
    WlanDebug(WlanMsg, "SCAN_RESP: returned %d APs before parsing\n",pScan->NumberOfSets);
     ...
     */
    	...
    	if((pScan->NumberOfSets==1) && bytesLeft) 
	{
        	/* Zero out the newBssEntry we are about to store info in */
        	rt_memset(newBssEntry, 0x00, sizeof(newBssEntry));
             ...
             *   possible
             */
            	//wlan_ret_802_11_scan_store_beacon(card,newBssEntry);
             ...
             */
            if (pTsfTlv) {
                rt_memcpy(tsfVal, &pTsfTlv->tsfTable[0], sizeof(tsfVal));
                rt_memcpy(&newBssEntry->networkTSF[0],
                       tsfVal, sizeof(newBssEntry->networkTSF));
            }
		hexdump("scan TSF:",(unsigned char * )pTsfTlv->tsfTable,8);
		hexdump("newBss TSF:",(unsigned char * )newBssEntry->networkTSF,8);
            /* Copy the locally created newBssEntry to the scan table */
            card->ScanTable=newBssEntry;

        } else {

	     rt_free(newBssEntry);
			/* Error parsing/interpreting the scan response, skipped */
            WlanDebug(WlanErr, "SCAN_RESP: "
...
{	
	...
	if(((itemcount+1)*sizeof(ScanResultItem)) >card->ScanResulMuxsize)
	{
    	...
    	*bytesLeft -= beaconSize;
    	...
    	pCurrentPtr += 2;
    	bytesLeftForCurrentBeacon -= 2;
	 WlanDebug(WlanMsg,"capability %x\n",pCap);
	  WlanDebug(WlanMsg,"Privacy %x\n",pCap->Privacy);
    	if (pCap->Privacy) {
    	...
    	while (bytesLeftForCurrentBeacon >= 2) {
        	...
        	case COUNTRY_INFO:
            		...
            		if ((pcountryinfo->Len < sizeof(pcountryinfo->CountryCode))||(pcountryinfo->Len > 254)) {
                		WlanDebug(WlanMsg,"ParzeBSSD: 11D- Err CountryInfo len =%d min=%d max=254\n", 
                       			pcountryinfo->Len, sizeof(pcountryinfo->CountryCode));
   
                	return WLAN_STATUS_FAILURE;
            		}
				WlanDebug(WlanEncy,"ParzeBSSD: country: 0x%x\n",pcountryinfo->CountryCode[0]);
				WlanDebug(WlanEncy,"ParzeBSSD: country: 0x%x\n",*(u16*)&pcountryinfo->CountryCode[1]);
				WlanDebug(WlanEncy,"ParzeBSSD: : 0x%x\n",pcountryinfo->Subband[1].FirstChan);
				WlanDebug(WlanEncy,"ParzeBSSD: : 0x%x\n",pcountryinfo->Subband[1].MaxTxPwr);
				WlanDebug(WlanEncy,"ParzeBSSD: : 0x%x\n",pcountryinfo->Subband[1].NoOfChan);
#endif
            		break;
        	case ERP_INFO:
			break;
