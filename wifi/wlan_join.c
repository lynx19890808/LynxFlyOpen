...
int wlan_cmd_802_11_associate_cfg(WlanCard *cardinfo, WlanConfig* cfg)
{
	...
	cfg->Ssid.SsidLength = strlen(cfg->Ssid.Ssid);
	rt_memcpy(cfg->Ssid.Ssid, cfg->SSID, WLAN_MAX_SSID_LENGTH);
	...
	pSsidTlv->Header.Type = (TLV_TYPE_SSID);
	pSsidTlv->Header.Len = cfg->Ssid.SsidLength;
	rt_memcpy(pSsidTlv->SsId, cfg->Ssid.Ssid, pSsidTlv->Header.Len);
	temlen += sizeof(pSsidTlv->Header) + pSsidTlv->Header.Len;
	rt_memcpy(pos, (u8*) pSsidTlv, temlen);
	pos += sizeof(pSsidTlv->Header) + pSsidTlv->Header.Len;
	...
	rt_free(tempuf);
	tempuf = NULL;
	// wlan_cmd_append_tsf_tlv_cfg(card, &pos, cfg);
