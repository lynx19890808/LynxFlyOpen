...
/********************************************************
 Global Variables
 ********************************************************/
 extern int wlan_set_universaltable(WlanCard *cardinfo, u8 band);
 extern int WlanparserScanResult(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp);
 extern int wlan_ret_802_11_scan(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp);
 extern int wlan_ret_802_11_associate(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp);
...
static int wlan_ret_get_hw_spec(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp)
{
	...
	rt_memcpy(card->wlandev->dev_addr, card->MyMacAddress, ETH_ALEN);
...
wlan_ret_802_11_rssi(WlanCard *cardinfo, HostCmd_DS_COMMAND * resp)
{
    ...
    //WlanCard *card = cardinfo;  猞猁注释掉了 因为编译警告 20131103
...
int wlan_process_event(WlanCard *card)
{
	...
	case MACREG_INT_CODE_DEAUTHENTICATED:
		...
		StateFlag |= STATE_COMM_FAIL;  //连接失败，更改标志位 由于不知道其他事件的含义，只能加在了这里 猞猁增加20131015
		break;
